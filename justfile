...
full-stack:
    colima start --kubernetes && sed -e "s|\${GNEWS_API_KEY_PLACEHOLDER}|$GNEWS_API_KEY|g" -e "s|\${GNEWS_BASE_URL_PLACEHOLDER}|$GNEWS_BASE_URL|g" full-stack.yaml | kubectl apply -f - && (while ! kubectl port-forward service/news-api-service 8000:80; do sleep 3; done)
